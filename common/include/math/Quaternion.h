...
public:
  ...
  /// Constructs a Quaternion object from v.
  HOST DEVICE
  template <typename V>
  explicit Quaternion(const V& v):
    x{real(v.x)},
    y{real(v.y)},
    z{real(v.z)},
    w{real(v.w)}
...
using quatf = cg::Quaternion<float>;
using quatd = cg::Quaternion<double>;
